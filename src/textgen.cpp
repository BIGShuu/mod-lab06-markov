...
prefix TextGenerator::build_prefix(const prefix& p, const std::string& word) const {
    prefix new_prefix = p;
...
std::string TextGenerator::choose_suffix(const prefix& p) const {
...
prefix TextGenerator::make_prefix(const std::vector<std::string>& words) const {
    prefix p;
    for (size_t i = 0; i < std::min(words.size(), static_cast<size_t>(NPREF)); ++i) {
        p.push_back(words[i]);
    }
    return p;
}
std::string TextGenerator::generate(int max_words) {
    ...
    std::uniform_int_distribution<size_t> start_dist(0, states.size() - 1);
